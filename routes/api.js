...
router.get('/spamcall', async (req, res, next) => {
        ...
        if(!apikeyInput) return res.json(loghandler.notparam)
        if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!no) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter no=8xxxxxxxx"})

       fetch(encodeURI(`https://id.jagreward.com/member/verify-mobile/${no}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
                 status : true,
                 creator : `Fhans`,
                 message : `owner nya badut + bahan pelampiasan;)`
             })
         })
         .catch(e => {
                res.json(loghandler.error)
